...
for(one.species in species) {
  ...
  ## loop to choose the best hits for each query (target species) gene
  
  t.print("loop to choose the best hits for each query (target species) gene")
  ...
  # how genes without assigned orthologues do we have?
  stats.ortho[11, "legend"] <- "Genes without orthologues/homologues"
  stats.ortho[11, one.species] <- sum(is.na(ortho.big.5$Arabidopsis_thaliana))
...
dir.create(path = "supplemented_orthologues/stats", recursive = T, showWarnings = F)
write.table(x = stats.ortho, file = "supplemented_orthologues/stats/supplementing_orthologues_stats.tsv", sep = "\t", row.names = F)
