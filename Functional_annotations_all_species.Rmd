...
## Some checks
```{r}
sum(is.na(short.ann$Arabidopsis_thaliana))
sum(grepl(pattern = "AT", x = short.ann$Arabidopsis_thaliana))
dim(short.ann)
...
all(short.ann$Arabidopsis_arenosa %in% prot.length$gene)
all(prot.length$gene %in% short.ann$Arabidopsis_arenosa)
prot.length$gene[!prot.length$gene %in% short.ann$Arabidopsis_arenosa]

aa <- read.table(file = "supplemented_orthologues/Arabidopsis_arenosa__v__Arabidopsis_thaliana_supplemented.tsv", header = T, sep = "\t", na.strings = "",
												comment.char = "", quote = "")
dim(aa)

bb <- readRDS("functional_annotation/4_full_rds/Arabidopsis_arenosa_At_orthologues_and_functional_annotations_full.rds")
dim(bb)
```
